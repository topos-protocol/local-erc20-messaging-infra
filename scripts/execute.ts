...
// Usage:
// ts-node ./scripts/execute.ts <source node endpoint> <target node endpoint> <sender private key> <tx hash> <indexes>
// Example: ts-node ./scripts/execute.ts http://localhost:8545 http://localhost:8546 0x... 0x... 0
const main = async function (...args: string[]) {
  const [
    sourceProviderEndpoint,
  ...
  console.log(`Executing transaction ${txHash} with indexes [${indexes}]...`);
  const numArray: number[] = [Number(indexes)];

  const executeTx = await erc20Messaging.execute(
    numArray,
    proofBlob,
  ...
  try {
    ...
    if (receipt.status === 1) {
      ...
      console.log(`txHash=${executeTx.hash}`);
    }
  } catch {
    ...
    try {
      ...
      if (error.data) {
        const decodedERC20MessagingError = new Interface(
          erc20MessagingJSON.abi
        ).parseError(error.data);
        if (decodedERC20MessagingError !== null) {
          console.error(
            `Transaction failed with custom error: ${
              decodedERC20MessagingError!.name
            }`
          );
          process.exit(1);
        }
        const decodedERCTokenError = new Interface(
          burnableMintableCappedERC20JSON.abi
        ).parseError(error.data);
        if (decodedERCTokenError !== null) {
          console.error(
            `Transaction failed with custom error: ${
              decodedERCTokenError!.name
            }`
          );
          process.exit(1);
        }
...
async function getReceiptMptProof(txHash: string, provider: AbstractProvider) {
